...
export default function LocationBar({ onLocationReady }) {
  ...
  const handleGeocode = async () => {
    ...
    try {
      ...
      await save(parseFloat(lat), parseFloat(lon), 'manual', shortName)
      setEditing(false)
      setQuery('')
    } catch {
  ...
  const locationLabel = () => {
    ...
    if (status === 'manual' && location) return (
      <span className="text-cyan-700 font-medium">
        📍 {locationName ? locationName : `${location.lat.toFixed(3)}, ${location.lng.toFixed(3)}`}
      </span>
    )
    ...
    <div className="flex items-center flex-wrap gap-2 px-3 py-2 bg-cyan-50 border border-cyan-100 rounded-xl mb-5 text-xs text-gray-500">
      ...
      {!editing && (
        <div className="ml-auto flex items-center gap-2">
          <button onClick={detect} disabled={status === 'detecting'}
