...
function AIChat() {
  ...
  // session_id is generated by the backend on the first request and returned
  // in the response. We store it here and send it back with every subsequent
  // request so the backend can look up this conversation's history.
  ...
  const handleSend = async () => {
    ...
    try {
      // Pass session_id (empty string on first message — backend will create one)
      const res = await askAI(input, medicineNames, sessionIdRef.current || '')

      // Save the session_id returned by the backend for all future requests
