            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                <div className="xl:col-span-2 space-y-4">
                  ...
                  {current && (
                    <div className="bg-gradient-to-br from-cyan-500 to-cyan-700 rounded-2xl shadow-md p-6 text-white">
                      ...
                      <button onClick={() => setCurrentIndex(i => Math.min(i + 1, queue.length - 1))}
                        className="w-full mt-5 bg-white text-cyan-600 font-bold text-sm py-2.5 rounded-xl hover:bg-cyan-50 transition-colors">
                        Next Patient →
                      </button>
                    </div>
                  )}
                      ...
                      <div className="divide-y divide-gray-50">
                        {waiting.map((p, i) => (
                          <div key={p.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-gray-50">
                            <div className="w-7 h-7 rounded-full bg-gray-100 text-gray-500 text-xs font-bold flex items-center justify-center shrink-0">{currentIndex + i + 2}</div>
                ...
                <div className="space-y-4">
                  ...
                  {done.length > 0 && (
                    <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                      <div className="px-5 py-4 border-b border-gray-100"><h3 className="text-sm font-bold text-gray-800">Completed</h3></div>
                      <div className="divide-y divide-gray-50">
                        {done.map(p => (
